...
const onMysqlBackupHandler = async (req, res) => {
  try {
    ...
    const backupFilePath = path.join(
      backupFolderPath,
      `backup_${"webdataconversion_2"}_${timestamp}.sql`
    );
    ...
    const command = `${mysqldumpPath} -u ${process.env.SQL_USER} -p${
      process.env.SQL_PASS
    } ${process.env.SQL_DATABASE_NAME} > "${backupFilePath.replace(
      /\\/g,
      "\\\\"
    )}"`;
    ...
    exec(command, (error, stdout, stderr) => {
      if (error) {
        ...
        return res
          .status(500)
          .json({
            message: "An error occurred during MySQL backup",
            error: stderr,
          });
      }
